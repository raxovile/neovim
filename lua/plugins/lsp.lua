return {
	-- Haupt-LSP-Konfiguration
	{
		...
		dependencies = {
			...
			-- Anzeige des LSP-Status
			{ "nvim-lua/lsp-status.nvim" }, -- **Stellen Sie sicher, dass dieser Name korrekt ist**
		...
		config = function()
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(event)
					...
					map("<leader>ds", require("telescope.builtin").lsp_document_symbols, "[S]ymbole")
					map("<leader>ws", require("telescope.builtin").lsp_workspace_symbols, "[W]orkspace")
			...
			local servers = {
				...
				omnisharp = {
					cmd = {
						tools_path .. "/omnisharp/OmniSharp.exe",
						"--languageserver",
						"--hostPID",
				...
				powershell_es = {
					cmd = {
						...
						"-NoProfile",
						"-Command",
						tools_path .. "/PowerShellEditorServices/PowerShellEditorServices/Start-EditorServices.ps1",
					},
